...
<title>Insert title here</title>
   <link rel="Stylesheet" href="<%=request.getContextPath()%>/style/default.css" /></head>
<body>
				...
				<c:forEach var="board" items="<%=list %>">
					<tr onmouseover="this.style.backgroundColor='gray'" onmouseout="this.style.backgroundColor='white'">
						...
						<td align="left">
							...
							<a href="<%= request.getContextPath() %>/board_content.meta?idx=${board.idx}&cp=${cpage}&ps=${pagesize}">
								<c:choose>
					...
					-->
						...
						<c:forEach var="i" begin="1" end="${pagecount}" step="1">
							<c:choose>
								...
								<c:otherwise>
									<a href="<%= request.getContextPath()%>/board_list.meta?cp=${i}&ps=${pagesize}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<!--다음 링크 --> 
						<c:if test="${cpage < pagecount}">
							<a href="<%= request.getContextPath()%>/board_list.meta?cp=${cpage+1}&ps=${pagesize}">다음</a>
						</c:if>
					</td>
