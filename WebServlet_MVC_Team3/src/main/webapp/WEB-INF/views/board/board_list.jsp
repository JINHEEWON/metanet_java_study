			...
			<table width="80%" border="1" cellspacing="0" align="center">
					...
					<tr onmouseover="this.style.backgroundColor='gray'" onmouseout="this.style.backgroundColor='white'">
						...
						<td align="left">
							...
							<c:if test="${board.depth > 0}">
								<img src="${pageContext.request.contextPath}/WEB-INF/images/re.gif">
							</c:if>
							
							<a href="${pageContext.request.contextPath}/board_content.meta?idx=${board.idx}&cp=${cpage}&ps=${pagesize}">
								<c:choose>
				...
				<tr>
					...
					-->
						...
						<!--이전 링크 --> 
						<c:if test="${cpage > 1}">
							<a href="${pageContext.request.contextPath}/board_list.meta?cp=${cpage-1}&ps=${pagesize}">이전</a>
						</c:if>
						<!-- page 목록 나열하기 -->
						<c:forEach var="i" begin="1" end="${pagecount}" step="1">
							<c:choose>
								...
								<c:otherwise>
									<a href="${pageContext.request.contextPath}/board_list.meta?cp=${i}&ps=${pagesize}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<!--다음 링크 --> 
						<c:if test="${cpage < pagecount}">
							<a href="${pageContext.request.contextPath}/board_list.meta?cp=${cpage+1}&ps=${pagesize}">다음</a>
						</c:if>
					</td>
