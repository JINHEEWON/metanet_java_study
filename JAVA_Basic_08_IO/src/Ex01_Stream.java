...
public class Ex01_Stream {
	...
	public static void main(String[] args) {
		...
		byte[] outSrc = null;	//현재 메모리를 갖고 있지 않음
		...
		while((data = input.read()) != -1) {
			...
			output.write(data);
			//write 대상이 data가 아니고 data를 ByteArrayOutputStream 자신의 통로에 write
		}
		
		outSrc = output.toByteArray();	//heap에 생성된 배열 객체의 주소값 return
		...
		while(input.read() != -1) {
			...
			//input.read() >> next   ->    read()를 한번만 써야 모든 데이터를 읽을 수 있음
		}*/
	}
