...
public class Ex06_File_SubList {
	...
	static void printFileList(File dir) {
		...
		//폴더개수
		int dirnum = subdir.size();		//현재 주어진 C:\temp  하위에 있는 폴더 개수
		//파알개수
		int filenum = files.length - dirnum;		//C:\temp  하위에 있는 파일 개수
		 ...
		 */
		System.out.println("****************************");
		//List<Integer> subdir = new ArrayList<Integer>();	파일 객체의 폴더가 있는 방번호 저장
		//[0] > 1
		//[1] > 2
		for(int i=0; i<subdir.size(); i++) {
			int index = subdir.get(i);
			printFileList(files[index]);	//재귀처럼
		}
	}
