/*
 ...
 결과: stack 여러개 생성해서 >> 동시에 함수를 실행 (실행가능하게 한다) >> cpu 점유할 수 있는 상태로 만든다
...
class Task_1 extends Thread {
	@Override
	public void run() {	//이 함수는 Thread의 main() 역할 >> 새로운 stack이 만들어지면 처음 올라가서 실행되는 함수
...
class Task_2 implements Runnable {		//Thread 아님 (run() 가지고 있는 클래스일 뿐)
...
public class Ex02_Multi_Thread {
	...
	public static void main(String[] args) {
		//main Thread
		Task_1 th = new Task_1();	//Thread객체 생성
		th.start();		//*POINT* 메모리를 만드는 역할. start() 호출하는 순간 stack 하나 생성해주고 stack에 run()까지 올리면 역할 끝. 소멸됨
		//start하지 않으면 새로운 메모리가 올라가지 않음
